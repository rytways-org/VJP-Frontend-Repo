...
function MaterialRequestSearch(props) {
  ...
  const loadInitialmaterialPRs = useCallback(async () => {
    ...
    const initialProds = await post(api + "/materialPr/prS",{"random":Math.random()});
    const reqNo = await post(api + "/materialPr/loadReqNo",{"random":Math.random()});
    const currentYear = new Date().getFullYear();
    if (response.ok) 
      setMaterialPRs(initialProds);
      setRequestNo(`PR-${currentYear}-000${Number(reqNo)+1}`);
  ...
  return (
    <div className={classes.container}>
      <SearchCard
        title="Material Purchase Request Search"
        buttonName="New PR"
