...
export const ProductTable = (showFormHandler,actions) => {
  return [
    ...
    },
    {
      ...
      render: rowData => {
        return (
          <> {["Generate"].includes(`${rowData.status}`) ? <span style={{cursor:"pointer",color:"blue"}} onClick={showFormHandler(rowData,actions[2])}>{rowData.status}</span> : <span>{rowData.status}</span>}
           </>)
      },
    },
      {
